...
try {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS reviews (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      orderId INTEGER NOT NULL,
      targetId INTEGER NOT NULL, -- product, courier or agent ID
      targetType TEXT NOT NULL, -- 'product', 'courier', 'agent'
    ...
    INSERT OR IGNORE INTO settings (key, value) VALUES ('warehouse_lat', '39.7747');
    INSERT OR IGNORE INTO settings (key, value) VALUES ('warehouse_lng', '64.4286');
  ...
  try { db.prepare("ALTER TABLE banners ADD COLUMN images TEXT").run(); } catch (e) { }
  try { db.prepare("ALTER TABLE products ADD COLUMN costPrice REAL DEFAULT 0").run(); } catch (e) { }
  try { db.prepare("ALTER TABLE products ADD COLUMN rating REAL DEFAULT 0").run(); } catch (e) { }
  try { db.prepare("ALTER TABLE products ADD COLUMN rating_count INTEGER DEFAULT 0").run(); } catch (e) { }
  try { db.prepare("ALTER TABLE users ADD COLUMN rating REAL DEFAULT 0").run(); } catch (e) { }
  try { db.prepare("ALTER TABLE users ADD COLUMN rating_count INTEGER DEFAULT 0").run(); } catch (e) { }
  try { db.prepare("ALTER TABLE orders ADD COLUMN is_rated INTEGER DEFAULT 0").run(); } catch (e) { }
  try {
  ...
  try {
    // Just in case existing deploy needs the new table created individually
    ...
    db.prepare("ALTER TABLE shops ADD COLUMN isArchived INTEGER DEFAULT 0").run();
  } catch (e) { }

  try {
    db.prepare("INSERT OR IGNORE INTO settings (key, value) VALUES ('warehouse_lat', '39.7747')").run();
    db.prepare("INSERT OR IGNORE INTO settings (key, value) VALUES ('warehouse_lng', '64.4286')").run();
  } catch (e) { }

  try {
    db.prepare("ALTER TABLE debts ADD COLUMN payerName TEXT").run();
  } catch (e) { }
} catch (error) {
