...
export const updateOrder = (req, res) => {
  ...
  try {
    ...
    if (courierId) {
      const courier = db.prepare("SELECT phone FROM users WHERE id = ?").get(courierId);
      if (courier) {
        ...
        if (botToken && chatId) {
          fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
            ...
            body: JSON.stringify({ chat_id: chatId, text: `🚚 <b>Новый заказ #${req.params.id}</b>\nНазначен курьеру: ${courier.phone}`, parse_mode: 'HTML' })
