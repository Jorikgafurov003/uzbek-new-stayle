...
export const createProduct = (req, res) => {
  const { name, price, discountPrice, categoryId, image, images, videoUrl, description, stock } = req.body;
  try {
    const result = db.transaction(() => {
      const info = db.prepare("INSERT INTO products (name, price, discountPrice, categoryId, image, videoUrl, description, stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(
